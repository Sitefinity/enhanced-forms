(function () {
    ...
    simpleViewModule.controller('SimpleCtrl', ['$scope', 'propertyService', '$q', function ($scope, propertyService, $q) {
        ...
        propertyService.get()
            .then(function (data) {
                if (data) {
                    ...
                    $scope.statesByCountry = JSON.parse($scope.properties.Model.ActiveCountriesWithStatesSerialized.PropertyValue);
                    $scope.countries = Object.keys($scope.statesByCountry);
                }
            })
            .catch(function (data) {
        ...
        $scope.addItem = function () {
            ...
            // scroll to bottom when adding new item
            setTimeout(function () {
        ...
        $scope.countryChanged = function () {
            var states = $scope.statesByCountry[$scope.properties.Model.SelectedCountry.PropertyValue];
            if (states && states.length) {
                $scope.currentItems = [$scope.currentItems[0], ...states];
            }
        }
    }]);
})();